...
- `governance.bypass.principals` — privileged-listener allow-list; principal-based, not client-id-based.
  ...
  - **Empty value is allowed but loud**: the default is the empty string, which parses to an empty `Set<String>`. RuleEngine logs a single `WARN` at construction time naming the consequence: with an empty bypass set, no principal can ride the privileged-listener bypass, and ALL traffic — including inter-broker (replica fetchers, KRaft metadata fetcher) — is subject to CEL rule evaluation. Combined with a deny-all rule this would soft-brick the cluster; the operator hazard surfaces in the startup log so it cannot be discovered only at request time.
