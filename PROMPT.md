...
- `governance.bypass.principals` — privileged-listener allow-list; principal-based, not client-id-based.
  ...
  - **Fail-fast on malformed entries**: any entry that fails `SecurityUtils.parseKafkaPrincipal` (no `:` separator), has an empty `principalType` (`":broker"`) or empty `name` (`"User:"`), or contains internal C0/C1/zero-width/bidi codepoints aborts broker startup with `IllegalArgumentException`. This matches upstream `StandardAuthorizer.super.users` parsing — no silent under-grant of the bypass. An operator typo fails startup loudly instead of producing an unreachable allow-list entry that silently rejects every peer.
...
The engine ships a hand-written recursive-descent parser, NOT cel-java. Supported: identifiers, field access (`a.b.c`), indexing (`arr[i]`, `map["key"]`), equality / comparison operators, logical AND/OR/NOT, integer literals, string literals (single- or double-quoted), boolean literals (`true` / `false`), the `null` literal, list literals (`[a, b, c]`), map literals (`{k: v}`), the `in` operator (typically against a list literal: `request.action in ["A", "B"]`), `exists` / `all` macros over Iterables, the `size(x)` function over Strings / Iterables / Maps, string `.startsWith/.endsWith/.contains/.matches(<regex literal>)`, integer arithmetic on `Long`s. Unsupported (rejected at parse): float literals, **bytes literals** (`b"…"` — the lexer has no `b"…"` form; the activation walker exposes byte-shaped scalars only as `{sizeInBytes: long}` descriptors, so the literal would be useless for comparison anyway), `dyn`, `has()`, `cel.bind`, timestamps, durations, user-defined functions, unbounded macros. The subset is intentional — it bounds the engine's blast radius and is the precondition for the static cost analysis that backs `MAX_NODES` / `MAX_EVAL_STEPS`.
