...
- **`KafkaHttpServerIntegrationTest`** — 29 tests against real embedded Jetty + Jetty `HttpClient` + Jetty `WebSocketClient`. Covers the PROMPT.md spec scenario (0 OK / 1 offline / 2 OK → 207) verbatim, Retry-After on quota throttle, ACL → 403, malformed JSON / missing records → 400, fetch HATEOAS with a cursor round-trip over the wire, deferred-future correctness, submitter throwable → 500, admission-control coverage (`produceWithOversizedBodyReturns413`, `sseReturns429WhenConcurrentStreamCapReached`, `wsReturns503WhenSubscriptionCapReached`), SSE replay-then-live plus partition-error coverage, and the WebSocket happy path including `wsSubscribeDeliversInitialCreditsThenFlowDeliversMore` (PROMPT.md FS2) and `wsClosesWith1003WhenFirstFrameIsNotSubscribe`.
- **`SocketServerConfigs`** — seven config keys for the bridge:
  ...
  - `http.bridge.max.concurrent.ws.subscriptions` (Int, default 100 — additional WebSocket upgrades beyond this cap are refused at the handshake with `503 Service Unavailable` + `Retry-After: 5`; set to 0 to disable WebSocket subscribe entirely while leaving produce / fetch / SSE open)
- **`KafkaConfig.scala`** — exposes all seven as `httpBridgeEnabled` / `httpBridgeHost` / `httpBridgePort` / `httpBridgeMaxRequestBodyBytes` / `httpBridgeRequestTimeoutMs` / `httpBridgeMaxConcurrentSseStreams` / `httpBridgeMaxConcurrentWsSubscriptions`.
...
4. **Concurrent WebSocket subscription cap → `503 Service Unavailable` (+ `Retry-After: 5`).** `http.bridge.max.concurrent.ws.subscriptions` (default 100) is enforced by `WsStreamLimiter`. The upgrade-time creator calls `wsLimiter.tryAcquire()` *before* constructing `KafkaWebSocketEndpoint` so a refused upgrade never enters the subscribe protocol against a connection that would immediately close. The status is `503`, not `429`, because the rejection happens at the upgrade handshake — there is no protocol state on the wire yet — and operator alerting reads them differently (`429` = "you are rate-limited"; `503` = "this listener is full right now"). Token ownership transfers to the endpoint at upgrade time and on to `WsStreamer` at the first valid `subscribe` frame; both holders' `close()` paths are idempotent so cap accounting cannot drift on errors / overlapping teardown.
...
|---|---|---|---|
| `RequestLatencyMs` | Histogram | `operation={Produce,Fetch}` | End-to-end servlet latency in ms (entry of `doPost`/`doGet` → completion of the async write). SSE-stream lifetimes are intentionally excluded — see below. |
...
| `RejectedAtWsCap` | Meter | — | Bridge-side 503 rejections at the WebSocket upgrade admission gate. WS uses 503 (rather than the SSE 429) because the rejection happens at the upgrade handshake — there is no subscribe protocol against a connection that would immediately close — and operator alerting typically distinguishes "you are rate-limited" (429) from "this listener is full right now" (503). |
