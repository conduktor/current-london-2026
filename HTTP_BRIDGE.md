...
The pure layer (`server/src/main/java/org/apache/kafka/network/http/`) is exhaustively unit-tested — 255 tests covering content-type negotiation, cursor encoding/decoding, request parsing edge cases, response formatting policy, Retry-After policy, error envelope shape, SSE framing, all four admission-control caps (body-size, request-timeout, concurrent-SSE, dispatch-off-handler-thread), and the new HTTP-level metrics layer (per-operation latency histograms, per-status-class meters, SSE-open/reject meters, active-SSE gauge — registered/unregistered cleanly across `KafkaHttpServer` lifecycles and stress-tested under concurrent close-vs-record). The broker integration is covered by seven real-broker end-to-end tests in `core/src/test/scala/integration/kafka/network/http/HttpBridgeEndToEndTest.scala` that boot a KRaft cluster via `KafkaClusterTestKit`, enable the bridge, and verify produce/fetch round-trips, mixed-partition 207, ACL-denied 403 on both POST (write) and GET (fetch) paths, SSE replay-then-live continuity, HAL+JSON cursor follow across pages with offset-contiguity assertion, and quota-throttle → 200 + `Retry-After` against a tight `producer_byte_rate` quota on `User:ANONYMOUS`.
