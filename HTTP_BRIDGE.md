...
- **`ProduceRequestParser`** — `POST` body `{records:[{partition (optional), key (optional), value (required)}]}` → `ProduceCommand`. Reuses `ValueSerializer.decode` for the envelope; the stored record `contentType` is **derived** from the `value` envelope's `type` field via `ValueSerializer.contentTypeFor` (JSON envelopes → `application/json`; everything else → null) rather than being a peer field of the record entry — a `contentType` key sent alongside `partition` / `key` / `value` is silently ignored. Throws `BadRequestException` for the 400 path.
...
- **`KafkaWebSocketEndpoint`** — the Jetty 12 `Session.Listener.AutoDemanding` implementation. One per upgraded connection. Owns the protocol state machine (OPEN → SUBSCRIBED → CLOSED), the limiter token (until ownership transfers to the streamer at the first valid subscribe frame), and the mapping from streamer-level failures to WebSocket close codes (`1003` for protocol violations, `1008` for the subscribe-deadline policy violation that fires when a client subscribes with zero credits and then stays silent past the deadline, `1011` for internal errors, `1001` for the broker-shutdown going-away path, `1000` for normal closure). The class is intentionally `public final` — Jetty's listener-method dispatch in `JettyWebSocketFrameHandlerFactory` resolves handlers through `MethodHandles.Lookup.unreflect()` from a different package and that lookup requires public class access, otherwise the upgrade attempt fails with `IllegalAccessException` surfaced as HTTP 500.
...
- **Leader-aware routing across the broker fleet.** The bridge runs in-process on every broker that sets `http.bridge.enabled=true` and synthesises every `ProduceRequest` / `FetchRequest` against the *local* broker's `RequestChannel` via `KafkaApiRequestSubmitter`. It does **not** look up partition leaders, does **not** issue redirects, and does **not** carry a `currentLeaderEpoch` (the submitter passes `Optional.empty()` so `FENCED_LEADER_EPOCH` is never raised on its behalf). A POST or GET whose target partition has its leader replica on a *different* broker therefore returns `503 SERVICE_UNAVAILABLE` with `errorCode=NOT_LEADER_OR_FOLLOWER` (or `LEADER_NOT_AVAILABLE` / `REPLICA_NOT_AVAILABLE` depending on which transient state the local `ReplicaManager` observes) and no `Retry-After` (the bridge does not know when leadership will move and a static hint would be misleading); on SSE the streamer emits a terminal error frame and closes the stream, and on WebSocket the endpoint sends an error envelope and closes with `1011`. Operators wanting transparent failover must either front the bridge ports with a partition-leader-aware load balancer (the standard Kafka client's responsibility, not a v1 bridge responsibility) or fall back to a standard Kafka client which performs `Metadata` lookup and leader discovery itself.
- **TLS termination on the bridge listener.** The bridge binds a plaintext HTTP/1.1 listener built from `HttpConnectionFactory` only — no `SslContextFactory`, no HTTP/2 / ALPN / h2c, no `ForwardedRequestCustomizer`. There is no `http.bridge.ssl.*` config surface in v1, and the listener will not respond to `Upgrade: h2c` (the request falls through to the standard HTTP/1.1 handler). Operators that need wire-level encryption between client and bridge, HTTP/2 multiplexing, or any X-Forwarded-* trust must front the bridge port with a TLS-terminating reverse proxy that forwards plaintext HTTP/1.1 to the bridge over a trusted interface — the same fronting proxy already required by Security model guardrail #2 for per-request authentication.
